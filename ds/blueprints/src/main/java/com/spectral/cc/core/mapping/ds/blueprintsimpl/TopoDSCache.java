...
public class TopoDSCache {
    ...
    protected static synchronized TopoDSCacheEntity getCachedEntity(String id) {
        return ddL2cache.get(id);
    }

    protected static synchronized void putEntityToCache(TopoDSCacheEntity entity) {
        Element element = entity.getElement();
        if (element instanceof Vertex) {
            ddL2cache.put("V" + entity.getElement().getProperty(TopoDSGraphPropertyNames.DD_GRAPH_VERTEX_ID), entity);
        } else if (element instanceof Edge) {
            ddL2cache.put("E" + entity.getElement().getProperty(TopoDSGraphPropertyNames.DD_GRAPH_EDGE_ID), entity);
        }
    }

    protected static synchronized void removeEntityFromCache(TopoDSCacheEntity entity) {
        Element element = entity.getElement();
        if (element instanceof Vertex) {
            ddL2cache.remove("V" + entity.getElement().getProperty(TopoDSGraphPropertyNames.DD_GRAPH_VERTEX_ID));
        } else if (element instanceof Edge) {
            ddL2cache.remove("E" + entity.getElement().getProperty(TopoDSGraphPropertyNames.DD_GRAPH_EDGE_ID));
        }
    }
