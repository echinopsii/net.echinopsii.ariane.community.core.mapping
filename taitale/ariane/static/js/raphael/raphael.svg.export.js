...
define(
    ...
    function (Raphael) {
                    ...
                    switch (name) {
                        ...
                        case 'fill':
                            if (element.match(/^hsb/)) {
                                ...
                                if (hsb.length === 3) {
                                    element = Raphael.hsb2rgb(hsb[0], hsb[1], hsb[2]).toString();
                                }
                            }
                    }
        ...
        Raphael.fn.toSVG = function() {
            var
                paper   = this,
                restore = { svg: Raphael.svg, vml: Raphael.vml },
                ...
                for ( i in node.attrs ) {
                    ...
                    switch ( i ) {
                        ...
                        case 'gradient':
                            ...
                            var fx = 0.5, fy=0.5;
                            gradient = String(gradient).replace(Raphael._radial_gradient, function (all, _fx, _fy) {
                                type = "radial";
                                if (_fx && _fy) {
                                    fx = parseFloat(_fx);
                                    fy = parseFloat(_fy);
                            ...
                            if(node.attrs.gradient.match(/^r/g)){
                                var dots = Raphael._parseDots(gradient);
                                if (!dots) {
                                    continue;
                                }
                                svg += '<defs>';
                                ...
                                var vector = [0, 0, Math.cos(Raphael.rad(angle)), Math.sin(Raphael.rad(angle))],
            ...
            Raphael.svg = restore.svg;
            Raphael.vml = restore.vml;

            return svg;
        };

        return this.RaphaelExport;
